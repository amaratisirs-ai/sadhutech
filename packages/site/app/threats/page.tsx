    ...
    return (
      <div className="flex items-center justify-center min-h-96">
        <div className="text-center space-y-4">
          ...
          <p className="text-slate-400 font-medium">Loading threat intelligence...</p>
        </div>
      </div>
    );
    ...
    return (
      <div className="space-y-6 max-w-6xl">
        <div className="bg-red-900/20 border border-red-800 rounded-lg p-6">
          <h2 className="font-bold text-red-200 mb-2">Error Loading Threats</h2>
          <p className="text-red-300 mb-4">{error}</p>
          <p className="text-sm text-red-400 mb-4">Community threat intelligence is temporarily unavailable. Please try again in a few moments.</p>
          <button
            onClick={() => {
              setError(null);
              fetchThreats(0);
    ...
    <div className="space-y-6 max-w-6xl">
      ...
      <div>
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-4xl font-bold text-white">Community Threat Intelligence</h1>
        </div>
      ...
      {stats && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="bg-slate-900/30 rounded-lg border border-slate-700 p-4">
            <div className="text-sm text-slate-400 mb-1">Showing Now</div>
            <div className="text-3xl font-bold text-white">{allThreats.length}</div>
          </div>
          <div className="bg-slate-900/30 rounded-lg border border-slate-700 p-4">
            <div className="text-sm text-slate-400 mb-1">High Risk Showing</div>
            <div className="text-3xl font-bold text-red-400">{allThreats.filter((t) => t.severity === "high").length}</div>
          </div>
          <div className="bg-slate-900/30 rounded-lg border border-slate-700 p-4">
            <div className="text-sm text-slate-400 mb-1">Confirmed by Community</div>
            <div className="text-3xl font-bold text-green-400">{allThreats.filter((t) => t.trusted).length}</div>
          </div>
        </div>
      )}

      {/* Sort & Filter Controls */}
      <div className="space-y-4 bg-slate-900/30 rounded-lg border border-slate-700 p-6">
        {/* Sort Buttons */}
        <div>
      ...
      <div className="space-y-2">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="grid grid-cols-4 gap-2 text-xs">
                    <div>
                      ...
                      <span className="font-semibold text-white">{threat.reports}</span>
                    </div>
                    <div>
                      <span className="text-slate-500">Reporters</span>{" "}
                      <span className="font-semibold text-white">{threat.reporters}</span>
                    </div>
                    <div>
                      <span className="text-slate-500">First Reported</span>{" "}
                      <span className="text-slate-300">{formatTime(threat.firstSeen)}</span>
                    </div>
                    <div>
                      <span className="text-slate-500">Last Reported</span>{" "}
                      <span className="text-slate-300">{formatTime(threat.lastSeen)}</span>
                    </div>
                  </div>
                  {threat.trusted && (
        ...
        {!hasMore && allThreats.length > 0 && (
          <div className="text-center py-4 text-slate-400">You&apos;re up to date with all {allThreats.length} reported threats.</div>
        )}
      </div>
    </div>
  );
