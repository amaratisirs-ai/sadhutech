  ...
  return (
    <html lang="en" className="h-full scroll-smooth">
      <head>
        ...
        <title>GENESIS Firewall — Pre-sign Gate Dashboard</title>
        <meta name="description" content="Community-powered transaction risk intelligence for crypto wallets." />
      </head>
      <body className="h-full bg-slate-950 flex flex-col pb-20 md:pb-0">
        <ThemeProvider>
                ...
                <div className="md:hidden border-t border-teal-500/20 bg-slate-900 py-4">
                  <div className="space-y-2">
                    <MobileNavLink href="/" onClick={() => setMobileMenuOpen(false)}>
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-3m0 0l7-4 7 4M5 9v10a1 1 0 001 1h12a1 1 0 001-1V9m-9 13l4-8m4 8L9 5" /></svg>
                      Home
                    </MobileNavLink>
                    ...
                    <MobileNavLink href="/snap-install" onClick={() => setMobileMenuOpen(false)}>
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m0 0h6m-6-6h-6" /></svg>
                      Get Protected
                    </MobileNavLink>
                    <MobileNavLink href="/threats" onClick={() => setMobileMenuOpen(false)}>
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>
                      Threats Hub
                    </MobileNavLink>
                    <MobileNavLink href="/community" onClick={() => setMobileMenuOpen(false)}>
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a3 3 0 00-5.856-1.487M15 10a3 3 0 11-6 0 3 3 0 016 0zM15 20H9m0 0v2a3 3 0 003 3h6a3 3 0 003-3v-2M9 10a3 3 0 116 0 3 3 0 01-6 0z" /></svg>
                      Community
                    </MobileNavLink>
                    <MobileNavLink href="/report" onClick={() => setMobileMenuOpen(false)}>
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8m0 8l-6-4m6 4l6-4" /></svg>
                      Report
                    </MobileNavLink>
                    ...
                    <MobileNavLink href="/api-explorer" onClick={() => setMobileMenuOpen(false)}>
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 20l4-16m4 4l4 4m0 0l-4 4m4-4H3" /></svg>
                      API Docs
                    </MobileNavLink>
                    <a
                      href="/snap-install"
          ...
          <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-slate-950 border-t-2 border-teal-500 backdrop-blur-xl z-40">
            <div className="flex justify-around items-center h-20">
              <BottomNavLink href="/" label="Home" icon={<svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 12l2-3m0 0l7-4 7 4M5 9v10a1 1 0 001 1h12a1 1 0 001-1V9m-9 13l4-8m4 8L9 5" /></svg>} />
              <BottomNavLink href="/threats" label="Threats" icon={<svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" /></svg>} />
              <BottomNavLink href="/snap-install" label="Protect" icon={<svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m0 0h6m-6-6h-6" /></svg>} />
              <BottomNavLink href="/report" label="Report" icon={<svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8m0 8l-6-4m6 4l6-4" /></svg>} />
              <BottomNavLink href="/help" label="Help" icon={<svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8.228 9c.549-1.165 2.03-2 3.772-2 2.21 0 4 1.343 4 3 0 1.4-1.278 2.575-3.006 2.907-.542.104-.994.291-.994.599 0 .11.023.207.066.286m0 0a9.75 9.75 0 01-.466 4.04m.466-4.04a9.75 9.75 0 001.457 4.04m0 0a9.75 9.75 0 001.457-4.04m0 0a9.75 9.75 0 01-.466-4.04" /></svg>} />
            </div>
          </nav>
...
function MobileNavLink({ href, children, onClick }: { href: string; children: ReactNode; onClick?: () => void }) {
  return (
    <a
      href={href}
      onClick={onClick}
      className="block px-4 py-3 text-base font-bold text-teal-300 hover:text-white hover:bg-teal-500/20 rounded-lg transition-all flex items-center gap-3 active:bg-teal-500/30"
    >
      {children}
    </a>
  );
...
function BottomNavLink({ href, label, icon }: { href: string; label: string; icon: ReactNode }) {
  return (
    <a
      href={href}
      className="flex flex-col items-center justify-center gap-1 w-full h-full text-teal-300 hover:text-white hover:bg-teal-500/20 transition-all group"
    >
